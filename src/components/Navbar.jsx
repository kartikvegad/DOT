...
const Navbar = () => {
    ...
    useEffect(() => {
        ...
        const onScroll = () => {
            ...
            const sections = [
                'services',
                ...(SITE_SECTIONS.showPricing ? ['pricing'] : []),
            ...
            <div className="nav-container">
                <div className="brand-wrapper">
                    <Link to="/" className="brand-link" onClick={closeMenu}>
                        ...
                        >
                            ...
                            <path d="M1268 427.61H1371V530.61H1268V427.61Z" fill="#4BC772" />
                        </svg>
                    </Link>
                </div>
                ...
                <div className="nav-right-group">
                    <div className={`nav-menu ${isMenuOpen ? 'open' : ''}`}>
                        ...
                        {SITE_SECTIONS.showPricing && sectionLink('pricing', 'Pricing')}
                        {SITE_SECTIONS.showTestimonials && sectionLink('reviews', 'Reviews')}
                        {sectionLink('work', 'Portfolio')}
                        {sectionLink('process', 'Process')}
                        {sectionLink('why-us', 'Why Us?')}
                        {sectionLink('stack', 'Stack')}
                        {SITE_SECTIONS.showTeam
                            ? sectionLink('team', 'Team')
                            : sectionLink('about', 'About Us')}
                        <Link
                            to="/contact"
                            className={`nav-item contact-btn ${isContactPage ? 'active' : ''}`}
                            onClick={() => {
                                window.scrollTo(0, 0);
                                closeMenu();
                            }}
